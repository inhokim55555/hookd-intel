...
function buildPrompt(ad: Ad, brand: BrandContext): string {
  const videoNote =
...
Brand Voice: ${brand.brand_voice || 'Not specified'}`
  ...
  return `Deconstruct this winning ad and create 10 completely distinct variations. Each must use a fundamentally different angle, emotional driver, and copy structure — not word swaps, but completely different strategic approaches.
...
Write exactly 10 variations using these angles in this order:
...
Write all 10. Do not summarize, abbreviate, or skip any field. Every variation must be complete and ready to hand to a copywriter or creative director.`
}

export async function POST(req: Request) {
  try {
    const body = await req.json()
    const { ad, brand_context } = body as {
      ad: Ad
    ...
    const userPrompt = buildPrompt(ad, brand_context ?? {})
